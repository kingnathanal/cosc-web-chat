...
async function connectChatSocket() {
    ...
    // Ensure we have a socket token to present during the handshake so the
    // server can authenticate the connection. If we already have one cached
    // use it, otherwise request a token from the API.
    if (!chatSocketToken) {
        try {
            const resp = await apiRequest('socket_token.php', { method: 'GET' });
            chatSocketToken = resp.token;
        } catch (err) {
            console.warn('Failed to obtain socket token', err);
            throw createSocketError('Unable to obtain socket token', err);
        }
    }

    const baseWsUrl = (chatSocketEndpoint && typeof chatSocketEndpoint === 'string' && chatSocketEndpoint.trim() !== '')
        ? chatSocketEndpoint
        : (() => {
    ...
    // Attach token as query param so the server can read it during handshake
    const sep = baseWsUrl.includes('?') ? '&' : '?';
    const wsUrl = `${baseWsUrl}${sep}token=${encodeURIComponent(chatSocketToken)}`;
